...
namespace KitchenCustomDifficulty
{
    public class EnforcePlayerBounds : Kitchen.EnforcePlayerBounds
    {
        ...
        protected override void OnUpdate()
        {
            ...
            if (allowOutOfBounds)
            {
                if (Has<SIsNightTime>() && Main.PrefSysManager.Get<int>(Main.PLAYER_COLLISION_PREP_ID) > 2)
                {
                    return;
                }
                if (Has<SIsDayTime>() && Main.PrefSysManager.Get<int>(Main.PLAYER_COLLISION_ID) > 2)
                {
                    return;
                }
            }
